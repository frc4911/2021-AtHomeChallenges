...
public class TestMode extends AutoModeBase {
    ...
    protected void routine() throws AutoModeEndedException {
        ...
        runAction(new SetTrajectoryAction(trajectories.slalomPathA.get(true), 0.0, 1.0));
        runAction(new RemainingProgressAction(0.5));

        runAction(new SetTrajectoryAction(trajectories.slalomPathB.get(true), 0.0, 1.0));
        runAction(new RemainingProgressAction(0.5));

        runAction(new SetTrajectoryAction(trajectories.slalomPathC.get(true), -360.0, 0.3));
        runAction(new RemainingProgressAction(0.5));

        runAction(new SetTrajectoryAction(trajectories.slalomPathD.get(true), -375.0, 1.0));
        runAction(new RemainingProgressAction(0.5));

        runAction(new SetTrajectoryAction(trajectories.slalomPathE.get(true), -375.0, 1.0));
