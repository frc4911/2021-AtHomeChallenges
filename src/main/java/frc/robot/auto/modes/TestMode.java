...
public class TestMode extends AutoModeBase {
    ...
    protected void routine() throws AutoModeEndedException {
        ...
        runAction(new setPoseAction(new Pose2d(0,0,new Rotation2d(45))));

        runAction(new SetTrajectoryAction(trajectories.slalomPathA.get(true), 0.0, .50));
        runAction(new RemainingProgressAction(.05));

        runAction(new SetTrajectoryAction(trajectories.slalomPathB.get(true), 0.0, 1.0));
        runAction(new RemainingProgressAction(.05));

        runAction(new SetTrajectoryAction(trajectories.slalomPathC.get(true), 180.0, .30));
        runAction(new RemainingProgressAction(.05));

        runAction(new SetTrajectoryAction(trajectories.slalomPathD.get(true), 180.0, 1.0));
        runAction(new RemainingProgressAction(.05));

        runAction(new SetTrajectoryAction(trajectories.slalomPathE.get(true), 90.0, .50));
